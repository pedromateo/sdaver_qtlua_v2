...
namespace qt {
    ...
    void Qt_GuiInterventor::doAction(const VerificationResult& vr)
    {
        ...
        foreach(RulePtr rulePtr,*(vr.failedRules))
        {
            ...
            //Checking that it is configured to paing shadow widgets on errors
        ...
        if (context_->interventionOnError()&VVL_GUI_SHOW_ERROR_RULES)
        {
            ...
            FailedRules::iterator it=failedRules_.begin();
            for (;it!=failedRules_.end();it++){
                //Insert into the proper validation shadow frame (indexed by the topLevelWidget of the widget that generated the error)
                //a new message with code it->first, generated by the widget it->second->interacted, and with it->second->related as the widgets related
