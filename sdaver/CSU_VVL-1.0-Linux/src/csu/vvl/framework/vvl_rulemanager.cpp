...
bool RuleManager::updateRules()
{
    ...
    else if (!filedatemap_.empty())
    {
        ...
        for(it = filedatemap_.begin(); it != filedatemap_.end(); it++)
        {
            // if changed
            if (_hasChanged((*it).first,(*it).second))
            {
        ...
        for(it_toReload = toReload.begin(); it_toReload != toReload.end(); it_toReload++){
            std::string file = *it_toReload;
            std::string basedir = filebasedirmap_[file];
            _reloadSingleFile(path(file),path(basedir));
        }
    }
    ///
...
bool RuleManager::_loadDirectory(boost::filesystem::path dir_path, boost::filesystem::path basedir)
{
    using namespace boost::filesystem;
    ...
    for (directory_iterator dir_itr(dir_path);dir_itr != end_iter;++dir_itr)
    {
        ...
        if(is_directory(dir_itr->status()))
        {
            ...
            if (dir_itr->path().filename().string()[0]!='.')
                _loadDirectory(dir_itr->path(),basedir);
        }else{
            _loadSingleFile(dir_itr->path(),basedir);
        }
    }
    return true;
}
...
///
void RuleManager::_loadRuleDirectories(StringVector paths_)
{
    using namespace boost::filesystem;
    ...
    _ilog::rules  << "(RuleManager::loadFiles) " << paths_.size() << " directories found to load" << std::endl;
    ...
    for(it = paths_.begin(); it < paths_.end(); it++)
    {
        ...
        if (is_directory(user_path))
        {
            ...
            _loadDirectory(user_path,user_path);

        } // end if
        else
        {
            _log::error  << "(RuleManager::loadFiles) No files allowed to load. Found file: "
                         << user_path.string() << std::endl;
            /*// the path is a single file
                _ilog::rules  << "(RuleManager::loadFiles) Found file: " << user_path.string() << std::endl;
                if (!_loadSingleFile(user_path))
                    context_->removeRuleSource((*it));*/
...
///
bool RuleManager::_loadSingleFile(boost::filesystem::path path, boost::filesystem::path basedir)
{
    char * buffer;
    int length;

    _ilog::rules  << "(RuleManager::loadSingleFile) loading single file: " << path.string() << std::endl;
    ...
    if (ifs.is_open())
    {
        ...
        for(it = functionDefs_.begin(); it != functionDefs_.end() ; it++)
        {
            ...
            for(;f_it1 != f_it2; ++f_it1){
                ...
                // Create the new rule
                _createNewRule(what[0].str(),what[2].str(),what[3].str(),path,basedir);
            }
        }
        // add the file to the class vector of files
        filedatemap_[path.string()] = _getFileLastDate(path);
        filebasedirmap_[path.string()] = basedir.string();
        delete[] buffer;
        _ilog::rules  << "(RuleManager::loadSingleFile) done." << std::endl;
        return true;
    }
    else
    {
...
///
void RuleManager::_reloadSingleFile(boost::filesystem::path path, boost::filesystem::path basedir)
{
...
///
bool RuleManager::_hasChanged(const std::string & file,const std::string & storedLastWrite)
{
    bool changed = false;

    std::string lastWrite = _getFileLastDate(file); // get the last write time
...
std::string RuleManager::_extractNameSpace(boost::filesystem::path path, boost::filesystem::path basedir)
{
    //
    // In this method we try to get the piece of path different from the basedir.
    // Then, with this piece we try to create a namespace for the widgets within a rule.
    //

    _ilog::rules  << "(RuleManager::extractNameSpace) Path original = " << path << std::endl;
    _ilog::rules  << "(RuleManager::extractNameSpace) Path basedir = " << basedir << std::endl;

    std::string result = "";
    std::string relativePath = path.branch_path().string();
    std::string relativeBasedir = basedir.branch_path().string();

    _ilog::rules  << "(RuleManager::extractNameSpace) relativePath = " << relativePath << std::endl;
    _ilog::rules  << "(RuleManager::extractNameSpace) relativeBasedir = " << relativeBasedir << std::endl;

    if (relativePath.empty()) return NULL;

    int pos = relativePath.find(relativeBasedir);
    assert(pos != std::string::npos);
    std::string workingPath = relativePath.size() > relativeBasedir.size()?
                relativePath.substr(relativeBasedir.size() + 1) : "";
    _ilog::rules  << "(RuleManager::extractNameSpace) Path working = " << workingPath << std::endl;

    typedef boost::tokenizer<boost::char_separator<char> > tokenizer;
    boost::char_separator<char> sep("/");
    tokenizer tokens(workingPath, sep);
    ...
    _ilog::rules  << "(RuleManager::extractNameSpace) Path result = " << result << std::endl;
    return result;
}
...
///
void RuleManager::_createNewRule(const std::string & rawRule,
                                 const std::string & id,
                                 ...
                                 boost::filesystem::path basedir)
{
    ...
    std::string formattedCode =
            ruleCodePreprocessor(id,ruleCode,widgets,_extractNameSpace(path,basedir));
...
///
std::string RuleManager::_getFileLastDate(boost::filesystem::path path)
{
...
{
    std::map<std::string, std::string>::iterator it;
    os << "printFiles: num files: " << filedatemap_.size() << std::endl;
