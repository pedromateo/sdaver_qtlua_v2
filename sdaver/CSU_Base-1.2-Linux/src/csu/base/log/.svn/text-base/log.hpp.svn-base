 ...
 *
 */
// $Id: log.hpp 4705 2010-07-23 10:33:47Z pedrolmn $
// $Revision: 4705 $
// $Date: 2010-07-23 12:33:47 +0200 (vie 23 de jul de 2010) $
#ifndef CSU_LOG_HPP
#define CSU_LOG_HPP
...
/// default null_id_value. This can become addictive
template <typename T>
...
// NOTE: This should be in a file by itself...
...
struct boost_threading_model
{
    ...
    struct boost_thread_id
    {
        ...
        {
            size_t operator()(thread_id_t const& id) const
            {
...
// Filter applicators
struct level_filter_applicator
{
    ...
    // Eval the exp with a given
    inline bool
    check (level_order_t o)
    {
        ...
        if (!environment_initted)
        {
            ...
            // their order)
            static_list<level_info>::plist& g =
                static_list<level_info>::list (NULL);
            static_list<level_info>::plist::const_iterator lit = g.begin();
            while (lit != g.end())
            {
                // Assign SYMBOL -> order (for levels)
                ee_.add_symbol ((*lit)->name_.c_str(),
                                s_symbol (new s_val ((int)(*lit)->order_)));
                ++lit;
            }
        ...
        const s_exp::s_val* result_v;
        try
        {
            result_v =
                s_exp::evaluator::eval<s_exp::eval_environment>(exp_, ee_);
        } catch (...) // eval_exception
        {
            result_v = NULL;
        }
        ...
        bool result;
        if (!result_v || result_v->t != s_exp::s_val::BOOL)
...
struct group_filter_applicator
{
    s_exp::eval_environment ee_;
    const s_exp::s_val* exp_;
    bool environment_initted;
    ...
    update(const group_filter& g)
    {
        ...
        s_exp::SemanticState ss;
        parser::State<s_exp::SemanticState> s
            (ss, g.filter_.c_str(), g.filter_.size());
        bool match = s_exp::grammar::gram::match(s);
        if (match)
            exp_ = new s_exp::s_val (ss.tok_stack().front());
        return match;
    ...
    check (group_id_t i)
    {
        ...
        if (!environment_initted)
        {
            ...
            // init the environment with the available levels (using
            // their order)
            static_list<group_info>::plist& g =
                static_list<group_info>::list (NULL);
            static_list<group_info>::plist::const_iterator git = g.begin();
            while (git != g.end())
            {
                // Assign SYMBOL -> group_id (for groups)
                ee_.add_symbol ((*git)->name_.c_str(),
                                s_symbol (new s_val ((int)(*git)->id_)));
                ++git;
            }
        ...
        // Add the current order as the special "_" parameter
        ee_.add_symbol ("_", s_exp::s_symbol (new s_exp::s_val ((int)i)));
...
struct logger_state
{
    ...
    // s-expr
    level_filter level_s_exp;

    // s-expr
    group_filter group_s_exp;
...
typedef detail::static_store_with_check<100,
                                        std::string,
...
namespace parser
{
using namespace csu::base::log::parser;
                                ...
                                string_p > >
{
    ...
    static inline void process_match (S& state, match_pair const&)
    {
        std::string s = detail::any_t::any_cast<std::string>(
            state.semantic_state().tok_stack().front());
        state.semantic_state().ls.group_s_exp = group_filter (s);
        state.semantic_state().tok_stack().pop_front();
    }
};
                              ...
                              string_p > >
{
    ...
    static inline void process_match (S& state, match_pair const&)
    {
        std::string s = detail::any_t::any_cast<std::string>(
            state.semantic_state().tok_stack().front());
        state.semantic_state().ls.level_s_exp = level_filter (s);
        state.semantic_state().tok_stack().pop_front();
    }
};
...
public:
    ...
    virtual logger_base& set_group_filter (group_filter const& f) = 0;

    virtual logger_base& set_level_filter (level_filter const& f) = 0;
...
public:
    ...
    ostream_ts_wrapper& operator<< (const std::string& s)
    {
        ...
        // around mutex_.
        csu::base::log::detail::scoped_lock<Lock> sl (mutex_);
        *stream_ << s << ::std::flush;
...
public:
    ...
    ostream_ts_wrapper_for(std::ostream& os)
    {
        ...
        return static_store<impl::default_stream_pool_size,
            std::ptrdiff_t,
            impl::null_id_value<std::ptrdiff_t>,
...
using detail::on;
using detail::off;
using detail::timestamp;
using detail::thread_id;
using detail::line_num;
using detail::w_level_name;
using detail::group_name;
using detail::log_format;
...
public:
    ...
    logger_base& set_group_filter (group_filter const&)
    {
        return *this;
    }

    logger_base& set_level_filter (level_filter const&)
    {
        return *this;
    }
    ...
    operator<< (logger<b,ThreadModel>& l, detail::modifier<T>)
    {
        return l;
    }
    ...
    /// Group filtering
    friend logger<b,ThreadModel>&
    operator<< (logger<b,ThreadModel>& l, group_filter const&)
    {
        return l;
    }
    ...
    /// Level filtering
    friend logger<b,ThreadModel>&
    operator<< (logger<b,ThreadModel>& l, level_filter const&)
    {
        return l;
    }
...
struct log_configuration
{
    ...
    void read_config_from_file (const std::string& file)
    {
        ...
        if (lc.is_open())
        {
            ...
            parser::State<conf::parser::SemanticState> s
                (ss, memblock, size);
            ...
            // Process all loggers
            static_list<logger_state>::plist& l =
                static_list<logger_state>::list (NULL);
            for (static_list<logger_state>::plist::const_iterator it = l.begin();
                 it != l.end();
                 ++it)
            {
...
struct log_tss
{
    ...
    log_tss()
        : actual_level (NULL), actual_group (NULL)
    {
    }
};
...
protected:
    ...
    // filter applicators
    level_filter_applicator lf_;
    group_filter_applicator gf_;
...
public:
    ...
    operator<< (logger<true,ThreadModel>& l, detail::modifier<T> m)
    {
        if (!l._may_do_output())
            return l;
    ...
    logger_base& set_group_filter (group_filter const& f)
    {
        d.group_s_exp = f;
        gf_.update (f);
        return *this;
    ...
    logger_base& set_level_filter (level_filter const& f)
    {
        d.level_s_exp = f;
        lf_.update (f);
        return *this;
    ...
    /// Group filtering
    friend logger<true,ThreadModel>&
    operator<< (logger<true,ThreadModel>& l, group_filter const& gf)
    {
    ...
    /// Level filtering
    friend logger<true,ThreadModel>&
    operator<< (logger<true,ThreadModel>& l, level_filter const& lf)
    {
    ...
    operator<< (logger<true,ThreadModel>& l, logger_base& (*o)(logger_base&))
    {
        ...
        if (o == detail::on || o == detail::off || l._may_do_output())
        {
            // apply the modifier
            (*o)(l);
        }
...
private:
    ...
    void _inject_timestamp (std::ostream& o)
    {
    ...
    std::string _build_log_line (std::string const& body)
    {
    ...
    void _output_data_to_streams (std::string const& data)
    {
        for(logger_state::OstreamList::iterator it = d.streams.begin();
            it != d.streams.end();
            ++it)
            {
                detail::ots_manager<Lock>::ostream_ts_wrapper_for(**it) << data;
            }
    }

    void _inject_thread_id(std::ostream& o)
    {
        // Add the thread Id
        o << '<' << ThreadID::this_thread_id() << '>';
    }

    log_tss& _tss_by_thread_id()
    {
